...
if (!content.includes('ledgerSupplier')) {
  const stateInsert = `
...
`;
  content = content.replace('const [editingSupplier, setEditingSupplier] = useState<any>(null);', stateInsert + '\n  const [editingSupplier, setEditingSupplier] = useState<any>(null);');
}

// 3. Add Modal UI right before the final </div> of the top section
          ...
          <DialogContent className="sm:max-w-3xl bg-white/95 backdrop-blur-xl border border-indigo-100 shadow-2xl rounded-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader className="border-b border-indigo-50/50 pb-4 mb-4 flex flex-row items-center justify-between">
              <div>
                ...
                <div className="text-sm text-slate-500 mt-1">Outstanding Balance: <span className={ledgerSupplier?.balance > 0 ? "text-orange-600 font-bold" : "text-emerald-600 font-medium"}>PKR {ledgerSupplier?.balance || 0}</span></div>
              </div>
...
content = content.replace('<DropdownMenuItem>View Ledger</DropdownMenuItem>', '<DropdownMenuItem onClick={() => handleViewLedger(supplier)}>View Ledger</DropdownMenuItem>');
